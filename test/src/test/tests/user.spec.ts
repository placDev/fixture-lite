...
describe('UserService (Integration)', () => {
    ...
    beforeEach(async () => {
        module = await generateTestingModule(
        ...
        const profile1 = await generator.entity(ProfileEntity).transform((item) => item).many(5);
        const profile2 = await generator.entity(ProfileEntity).transform((item) => item, []).many(5);
        const profile3 = await generator.entity(ProfileEntity).transform((item) => {
            item.userId = "555";
            return {};
        }, [1000]).many(5);
        ...
        await generator.entity(UserEntity).transform(async (item, generator) => {
            const user = item;
            const [profile] = await generator.entity(ProfileEntity).transform((item) => {
                item.userId = user.id;
                return item;
            }).accum();
